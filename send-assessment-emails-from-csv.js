...
// Customized for "Individual Sessions-Assessments.csv" format
// ============================================================
...
const CSV_FILE       = process.argv[2] || '/Users/sydnee/Documents/Individual Sessions-Assessments.csv';
...
async function readCSVFile(filePath) {
  return new Promise((resolve, reject) => {
    ...
    fs.createReadStream(filePath)
      .pipe(csv())
      .on('data', (row) => {
        // Map CSV columns to expected field names based on the actual CSV structure
        const record = {
          id: row['Record ID'] || Math.random().toString(36).substr(2, 9),
          fields: {
            'Client Name (from Client)': row['Client Name (from Client)'] || row['Client'] || '',
            'Parent Name (from Client)': row['Parent Name (from Client)'] || row['Parent Name (from Client) 2'] || '',
            'Email (from Client)': row['Email (from Client)'] || row['Email (from Client) 2'] || row['Email (from Client) 3'] || '',
            'Signature to LLC Liability (from Client)': row['Signature to LLC Liability (from Client)'] || '',
            'Start Date/Time for Email': row['Start Date/Time for Email'] || row['Start Date'] || '',
            'Address (from Session Group)': row['Address (from Session Group)'] || '1212 Kansas Ave, Modesto, CA 95351',
...
async function main() {
  ...
  const missing = allRecords.filter(r => {
    ...
    // If signature field is empty or null, waiver is missing
    return !sig || sig.trim() === '';
  });
  ...
  for (const r of missing) {
    ...
    if (!email) {
      const swimmerName = getCellValue(r, 'Client Name (from Client)') || 'Unknown swimmer';
      console.log(`⚠️  Skipping record with missing email for: ${swimmerName}`);
      continue;
    }
  ...
  for (const [email, records] of groups) {
    ...
    if (!DRY_RUN && email !== groups[groups.length - 1][0]) {
      await delay(1000);
    }
  }
