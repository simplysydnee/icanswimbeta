#!/usr/bin/env python3
"""
Modified Skill Tracker Data Import Script
Looks for CSV files in current directory or specified paths
"""
...
if missing_files:
    ...
    print("1. swimmer_uuid_mapping.csv (from Documents folder)")
    print("2. swimmer_skills.csv")
    print("3. swimmer_targets.csv")
    print("4. swimmer_strategies.csv")
    print("\nYou may need to copy them from /mnt/user-data/outputs/ or download them.")
    exit(1)
...
def import_swimmer_skills(uuid_mapping):
    ...
    print(f"Prepared {len(records)} skill records")
...
def import_swimmer_targets(uuid_mapping):
    ...
    print(f"Prepared {len(records)} target records")
...
def import_swimmer_strategies(uuid_mapping):
    ...
    print(f"Prepared {len(records)} strategy records")
    print(f"Skipped: {no_uuid} (no UUID)")
...
def main():
    print("=" * 60)
    print("SKILL TRACKER DATA IMPORT (Modified)")
    print("=" * 60)
