-- Fix: Decrement purchase_orders.sessions_booked when a funded client cancels
-- When a booking with an associated PO (funded client) is cancelled, we must
-- release the booked session back to the PO's available pool so the client
-- isn't charged for a cancelled lesson.
--
-- Logic:
--   - If the cancelled booking has a purchase_order_id, look up the linked
--     swimmer's funding source and verify requires_authorization = true
--   - If so, decrement sessions_booked (floor 0)
--   - This runs inside the existing FOR UPDATE lock on booking + session rows
...
BEGIN
  ...
  -- Decrement purchase order sessions_booked for funded clients
  -- Only if:
  --   1. Booking has an associated PO (v_booking.purchase_order_id)
  --   2. The booking was 'confirmed' (pending_auth bookings never incremented the PO)
  --   3. The funding source requires authorization
  IF v_booking.purchase_order_id IS NOT NULL AND v_booking.status = 'confirmed' THEN
    UPDATE purchase_orders
    SET sessions_booked = GREATEST(COALESCE(sessions_booked, 0) - 1, 0),
        updated_at = NOW()
    WHERE id = v_booking.purchase_order_id
      AND EXISTS (
          SELECT 1 FROM swimmers s
          JOIN funding_sources fs ON fs.id = s.funding_source_id
          WHERE s.id = v_booking.swimmer_id
            AND fs.requires_authorization = true
      );
  END IF;
