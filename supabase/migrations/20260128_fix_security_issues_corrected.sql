...
-- ============================================================================
-- PART 2: Recreate views with explicit WITH (security_invoker = true) (SAFE - views not used)
-- ============================================================================

-- Note: These views are NOT being used by the application, so recreating them
-- with WITH (security_invoker = true) won't break anything.
...
COMMENT ON VIEW public.swimmers_with_funding_details IS 'Swimmers with detailed funding information - WITH (security_invoker = true)';
COMMENT ON VIEW public.swimmers_with_funding IS 'Swimmers with basic funding information (legacy view) - WITH (security_invoker = true)';
COMMENT ON VIEW public.instructor_today_sessions IS 'Today''s sessions for current instructor - WITH (security_invoker = true) with auth.uid() filter';
COMMENT ON VIEW public.instructor_performance_dashboard IS 'Instructor performance metrics - WITH (security_invoker = true)';
COMMENT ON VIEW public.coordinator_performance_dashboard IS 'Coordinator performance metrics - WITH (security_invoker = true)';
