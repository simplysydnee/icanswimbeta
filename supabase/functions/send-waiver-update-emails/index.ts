...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (TEST_MODE) {
      console.log('[send-waiver-update-emails] TEST MODE ENABLED - emails will be logged but not sent');
    }
...
async function sendEmail({
  ...
  if (TEST_MODE) {
    console.log('TEST MODE - Would send waiver update email:');
    console.log('To:', to);
    console.log('Subject:', `Important: Update Liability Waivers for ${swimmers.join(', ')}`);
    console.log('Parent:', parentName);
    console.log('Swimmers:', swimmers.join(', '));
