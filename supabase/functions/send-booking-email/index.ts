...
interface EmailRequest {
  ...
  templateType:
    ...
    | 'assessment_booking'
    | 'recurring_lesson_booking'
    | 'single_lesson_booking'
...
const FROM_EMAIL =
  Deno.env.get('FROM_EMAIL') || 'I Can Swim <noreply@icanswim209.com>'
const APP_URL =
  Deno.env.get('APP_URL') || 'https://icanswim209.com'
const TEST_MODE = Deno.env.get('TEST_MODE') === 'true'

/* ---------------- TEMPLATE FUNCTION (UNCHANGED CORE) ---------------- */
...
serve(async (req) => {
  ...
  try {
    ...
    if (!to) {
      return new Response(
        JSON.stringify({ error: 'Missing "to" email' }),
