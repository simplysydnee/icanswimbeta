...
async function checkDbDataAuth() {
  console.log('=== CHECKING DATABASE DATA (WITH AUTH) ===\n');
  ...
  try {
    ...
    if (joinError) {
      ...
      // Try alternative: get skills and level separately
      console.log('\nTrying alternative query...');
      const { data: skillsOnly } = await supabase
        .from('skills')
        .select('*')
        .eq('level_id', alex.current_level_id)
        .order('sequence', { ascending: true });
      ...
      if (skillsOnly) {
        console.log(`✅ Got ${skillsOnly.length} skills without join`);
        // Manually attach level data
        const skillsWithManualLevel = skillsOnly.map(skill => ({
          ...skill,
          swim_levels: level
        }));
        console.log(`   First skill with manual level:`, skillsWithManualLevel[0]);
      }
    } else {
