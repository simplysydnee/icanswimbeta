// ============================================================
// I Can Swim — Urgent Assessment Waiver Email (CSV Version)
// Standalone Node.js version that reads from CSV file
// ============================================================
...
const DRY_RUN        = false; // Set to true for dry run, false for live mode
const CSV_FILE       = process.argv[2] || 'assessment_data.csv'; // CSV file path from command line
    ...
    fs.createReadStream(filePath)
      ...
      .on('data', (row) => {
        ...
        const record = {
          ...
          fields: {
            'Client Name (from Client)': row.client_name || row['Client Name'] || row.swimmer_name,
            'Parent Name (from Client)': row.parent_name || row['Parent Name'] || row.parent,
            'Email (from Client)': row.email || row['Email'] || row.parent_email,
            'Signature to LLC Liability (from Client)': row.signature_status || row.signature || row.waiver_signed,
            'Start Date/Time for Email': row.assessment_datetime || row.datetime || row.date_time,
            'Address (from Session Group)': row.address || row['Address'] || row.location,
            'Booking Status': row.booking_status || row.status || 'Confirmed'
          }
        };
        records.push(record);
      })
...
async function main() {
  ...
  console.log('I CAN SWIM - URGENT ASSESSMENT WAIVER EMAIL SCRIPT (CSV)');
  console.log('='.repeat(60));
  ...
  let allRecords;
  try {
    allRecords = await readCSVFile(CSV_FILE);
  } catch (error) {
    console.error(`❌ Failed to read CSV file: ${error.message}`);
    console.log('\nExpected CSV format (column names):');
    console.log('client_name,parent_name,email,signature_status,assessment_datetime,address,booking_status');
    console.log('\nExample row:');
    console.log('John Smith,Jane Smith,jane@example.com,,Tomorrow at 10:00 AM,"1212 Kansas Ave, Modesto, CA 95351",Confirmed');
    process.exit(1);
  }
  ...
  const missing = allRecords.filter(r => {
    ...
    // Consider empty, "false", "0", "no", "null", "undefined" as missing
    const sigValue = String(sig || '').toLowerCase().trim();
    return !sigValue ||
           sigValue === 'false' ||
           sigValue === '0' ||
           sigValue === 'no' ||
  ...
  for (const r of missing) {
    ...
    if (!email) {
      console.log(`⚠️  Skipping record with missing email for: ${getCellValue(r, 'Client Name (from Client)')}`);
      continue;
    }
  ...
  if (groups.length === 0) {
    console.log('❌ No valid email addresses found in missing waiver records.');
    process.exit(0);
  }
  ...
  for (const [email, records] of groups) {
    ...
    try {
      ...
      if (result.id && result.id !== 'dry-run-mock-id') {
  ...
  if (!DRY_RUN && sent > 0) {
    console.log(`\n📧 ${sent} email(s) were successfully sent via Resend.`);
  }
}
