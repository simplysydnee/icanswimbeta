#!/usr/bin/env python3
"""
Script to update Airtable records: mark 'Update email sent' = true for all delivered recipients.

Usage:
1. Set your Airtable personal access token as an environment variable:
   export AIRTABLE_PERSONAL_ACCESS_TOKEN="your_token_here"
   OR
   export AIRTABLE_TOKEN="your_token_here"

2. Run the script:
   python update_airtable_emails.py
"""
...
EMAIL_FIELD = "Email"  # Assuming the email field is named "Email"
...
# Delivered email list (184 addresses)
DELIVERED_EMAILS = {
    "1996.mzs@gmail.com",
...
def process_records(records: List[Dict]) -> Tuple[List[Dict], Set[str], Set[str], Set[str]]:
    ...
    # Track internal/test emails
    internal_emails = {"sydnee@simplysydnee.com", "christybalaoing@gmail.com", "t.ash5@yahoo.com"}
    internal_updates = []
    ...
    for record in records:
        ...
        # Get email from record (case-insensitive)
        ...
        if normalized_email in DELIVERED_EMAILS:
            ...
            current_value = fields.get(FIELD_ID, False)
...
def update_records(records_to_update: List[Dict]) -> List[Dict]:
    """Update records in Airtable."""
    if not records_to_update:
        print("\nNo records need updating.")
        return []

    print(f"\nUpdating {len(records_to_update)} records in Airtable...")

    updated_records = []
    failed_records = []

    # Update in batches (Airtable recommends max 10 records per request)
    batch_size = 10
    for i in range(0, len(records_to_update), batch_size):
        batch = records_to_update[i:i + batch_size]

        # Prepare batch update payload
        records_payload = [item["record"] for item in batch]
        ...
        try:
            ...
            for j, item in enumerate(batch):
                ...
                print(f"  ✓ Updated: {item['name']} ({item['email']})")
    ...
    if failed_records:
        print(f"\n⚠️  Failed to update {len(failed_records)} records")
        for item in failed_records:
            print(f"  ✗ Failed: {item['name']} ({item['email']})")

    print(f"\n✓ Successfully updated {len(updated_records)} records")
...
def print_summary(
    ...
    if updated_records:
        ...
        for item in updated_records:
            ...
            print(f"{prefix}{item['record']['id']}: {item['name']} ({item['email']})")
            if item["is_internal"]:
                internal_count += 1
...
def main():
    """Main function."""
    print("="*60)
    print("Airtable Email Update Script")
    print("="*60)
    ...
    # Process records
    records_to_update, emails_found, emails_not_found, emails_already_true = process_records(records)

    # Update records
    updated_records = update_records(records_to_update)

    # Print summary
    print_summary(updated_records, emails_found, emails_not_found, emails_already_true)
