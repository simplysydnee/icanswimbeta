...
async function fetchInstructorsWithTodaySessions(): Promise<InstructorWithSessions[]> {
  ...
  try {
    ...
    if (rolesError) {
      ...
      throw new Error('Failed to fetch instructor roles')
    }

    if (!instructorRoles || instructorRoles.length === 0) {
      console.log('=== DEBUG: No users with instructor role found')
      return []
    }
    ...
    if (instructorsError) {
      ...
      throw new Error('Failed to fetch instructor profiles')
    }
...
export default function StaffInstructorSelectFiltered() {
  ...
  const { data: instructors, isLoading, error } = useQuery({
    queryKey: ['staffInstructorsWithSessionsFiltered'],
