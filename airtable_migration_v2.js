// Airtable to Supabase Migration Script - Version 2
// Migrates "Actively Enrolled ✅" clients from Airtable to Supabase
// Uses parent_email field for later parent linking
...
function parseClientName(fullName) {
  ...
  // Clean name - remove markers and clean trailing dashes/spaces
  let cleanName = fullName
    .replace(/[(\[]P[)\]]/g, '')  // Remove (P) or [P]
...
function determineFundingSource(clientRecord, isPrivatePay) {
  ...
  const vmrcEmail = (clientRecord.fields['Email of VMRC Coordinator'] || '').toLowerCase();
  const vmrcClient = clientRecord.fields['VMRC Client'] || '';

  if (vmrcEmail.includes('vmrc') || vmrcClient === 'Yes' || vmrcClient === 'YES') {
    return 'vmrc';
  } else if (vmrcEmail.includes('cvrc')) {
    return 'cvrc';
  }
...
function mapAirtableToSwimmer(clientRecord, fundingSource) {
  ...
  // Track unique parent emails for reporting
  if (parentEmail) {
    stats.uniqueParentEmails.add(parentEmail.toLowerCase());
  }
  ...
  return {
    parent_id: null, // Will be linked later via parent_email
...
async function migrateClient(clientRecord) {
  try {
    ...
    const swimmerData = mapAirtableToSwimmer(clientRecord, fundingSource);
    ...
    await airtable(AIRTABLE_TABLE_ID).update(clientRecord.id, {
      ...
      'Migration Notes': 'Success - awaiting parent invite'
    });
...
async function runMigration() {
  console.log('Starting Airtable to Supabase migration (v2)...');
  console.log('===============================================\n');
  ...
  try {
    ...
    for (let i = 0; i < records.length; i += batchSize) {
      ...
      console.log(`  Unique parent emails: ${stats.uniqueParentEmails.size}`);
    ...
    console.log(`Successfully migrated: ${stats.totalMigrated}`);
    console.log(`Unique parent emails: ${stats.uniqueParentEmails.size} (invites to send)`);
    console.log(`Errors: ${stats.errors}`);
    ...
    // Save unique parent emails to file for reference
    const fs = require('fs');
    const emails = Array.from(stats.uniqueParentEmails).sort();
    fs.writeFileSync('parent_emails.txt', emails.join('\n'));
    console.log(`\nParent emails saved to: parent_emails.txt`);
