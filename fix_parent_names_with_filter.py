...
This script reads the CSV file and directly updates parent names in Supabase
only for swimmers with enrollment_status = 'pending_enrollment'.
"""
...
def process_csv_and_update(csv_path):
    """
    ...
    print("Filter: Only updating swimmers with enrollment_status = 'pending_enrollment'")
    print()
                        ...
                        try:
                            ...
                            if response.data and len(response.data) > 0:
                                ...
                                if swimmer.get('enrollment_status') == 'pending_enrollment':
                                    ...
                                    print(f"- Skipped (not pending_enrollment): {uuid_value} - status: {swimmer.get('enrollment_status')}")
                            else:
                                updates_skipped_no_match += 1
                                print(f"- Skipped (no matching swimmer): {uuid_value}")
...
def main():
    ...
    print("2. Find records with Supabase ID (UUID) and Parent Name")
    print("3. Update parent_name ONLY for swimmers with enrollment_status = 'pending_enrollment'")
    print("4. Report statistics")
    print()
    ...
    process_csv_and_update(csv_path)
